...
# newdemo in example_fc.py
class LIAFMLP(nn.Module):
    ...
    def __init__(self,config):
        #@param config.layerSize: a list of each layer's size with form:(n_1st,n_2nd,n_3rd,.....,n_nth)
        #@param config.actfun: handle of activation function(default is LIF mode, u can use any torch.act)
        ...
        #@param config.useThreshFiring: see details in forward()
        super().__init__()
        self.cfgFc=config.cfgFc
        self.timeWindows = config.timeWindows
        self.n_layer=len(self.cfgFc)-1#special
        ...
        self.batchSize = None
        for dice in range(self.n_layer):
            self.network.add_module(str(dice),
