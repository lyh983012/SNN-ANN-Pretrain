...
# Model for sharpened ReLU-ResNet
...
class Hardtanh(torch.autograd.Function):
    """sharpened ReLU"""
    @staticmethod
    def forward(ctx, input, min, max):
        ...
        b = (input > max).float()
        c = 1 - (a + b)
        ctx.save_for_backward(input, c, torch.Tensor([max-min]).cuda())
        return a * 0 + b * 1 + c * (input-min) / (max-min)

    @staticmethod
    def backward(ctx, grad_output):
        input, c, diff = ctx.saved_tensors
        grad_input = grad_output.clone()
        temp = abs(input - thresh) < lens
        temp = temp / (2 * lens)
...
def My_Hardtanh(input, min=0, max=1):
    return Hardtanh.apply(input, min, max)
...
class mem_update(nn.Module):
    ...
    def forward(self, x):
        if settings.IS_TRAIN is True:
            epoch = settings.EPOCH_INDEX
            ...
            for i in range(time_window):
                mem = x[i]
                spike = My_Hardtanh(mem, min=min(epoch/200, 0.49), max=1-epoch/200)
                output[i] = spike
        else:
            mem = torch.zeros_like(x[0]).to(device)
            spike = torch.zeros_like(x[0]).to(device)
