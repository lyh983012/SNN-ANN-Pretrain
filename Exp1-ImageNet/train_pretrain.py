...
def eval_training(epoch, args):
    ...
    if args.local_rank == 0:
        ...
        writer.add_scalar('Test/Average loss', test_loss * args.b / len(ImageNet_test_loader.dataset), epoch)
...
@torch.no_grad()
def eval_training_H(epoch, args):
    # eval for sharpened ReLU
    settings.IS_TRAIN = True
    start = time.time()
    net.eval()
    ...
    if args.local_rank == 0:
        # add information to tensorboard
        writer.add_scalar('Test/Average loss_Htanh', test_loss * args.b / len(ImageNet_test_loader.dataset), epoch)
        writer.add_scalar('Test/Accuracy_Htanh', correct.float() / real_batch * 100, epoch)
...
if __name__ == '__main__':
    ...
    optimizer = optim.SGD([{'params': net.parameters(), 'initial_lr': b_lr}], momentum=0.9, lr=b_lr, weight_decay=0)
    iter_per_epoch = len(ImageNet_training_loader)
    LOG_INFO = "Pretrain-stage"
    checkpoint_path = os.path.join(settings.CHECKPOINT_PATH, args.net, str(args.b), str(args.lr), LOG_INFO, settings.TIME_NOW)
    ...
    for epoch in range(1, settings.EPOCH+1):
        ...
        acc_H = eval_training_H(epoch, args)
