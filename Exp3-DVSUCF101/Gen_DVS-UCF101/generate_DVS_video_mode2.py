"""
generate DVS dataset mode-2

mean of contrast threshold (CT) is constant in each video clip spatially
contrast threshold (CT) is sampled from Normal Distribution in each pixel per video clip
while the mean of CT is dynamically adjusted in different video clips to guarantee the event ratio to be 0.15
"""
...
def gen_event_from_video(folder, configs):
    ...
    CT1 = np.random.randn(configs['num_frames'], H, W) * configs['gauss']['CT_std'] + ct1
    CT1[CT1 > -configs['gauss']['CT_min']] = -configs['gauss']['CT_min']
    CT2 = np.random.randn(configs['num_frames'], H, W) * configs['gauss']['CT_std'] + ct2
    CT2[CT2 < configs['gauss']['CT_min']] = configs['gauss']['CT_min']
