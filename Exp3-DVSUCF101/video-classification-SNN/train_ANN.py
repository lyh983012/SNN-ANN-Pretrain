"""
train LIAF-ANN
for convenient transfer from LIAF-ANN to LIAF and LIF
"""
...
from dataloader import Dataset_CRNN

from configs_ANN import training_configs, model_configs
...
ckpts_path = r'./checkpoints/ANN/'
tensorboard_path = r'./tensorboard/ANN/'
dic_save_path = r'./checkpoints/ANN/configs_ANN.txt'
log_path = r'./log/ANN/'
...
train_set = Dataset_CRNN(data_path, train_list, train_label, selected_frames, transform=train_transform)
test_set = Dataset_CRNN(data_path, test_list, test_label, selected_frames, transform=test_transform)
...
def train(configs, model, device, train_loader, optimizer, criterion, epoch, writer, f_log, running_loss_list):
    ...
    for batch_idx, (videos, labels) in enumerate(train_loader):
        videos, labels = videos.to(device), labels.to(device).view(-1, )
        optimizer.zero_grad()
        ...
        for t in range(videos.size(1)):
            frames = videos[:, t, :, :, :]
            B, C, H, W = frames.size()
            frames = frames.view(B, C, 1, H, W).to(device)
            features = cnn_encoder(frames)
            cnn_embed_seq.append(torch.squeeze(features))

        cnn_embed_seq = torch.stack(cnn_embed_seq, dim=0).transpose_(0, 1)
        outputs = rnn_decoder(cnn_embed_seq)
...
def main():
    ...
    with open(os.path.join(log_path, "ANN_screen_log.txt"), "w") as f1:
        for epoch in range(1, training_configs['total_epoch'] + 1):
            ...
            if test_acc > best_test_acc:
                ...
                f2 = open(os.path.join(log_path, "ANN_best_test_acc.txt"), "w")
                ...
                if epoch >= training_configs['total_epoch'] / 2:
                    ...
                    torch.save(checkpoints, os.path.join(ckpts_path, 'ann-epoch{}-best.pth.tar'.format(epoch)))
            ...
            if epoch % training_configs['save_interval'] == 0 or epoch == training_configs['total_epoch']:
                ...
                torch.save(checkpoints, os.path.join(ckpts_path, 'ann-epoch{}.pth.tar'.format(epoch)))

    writer.close()

    f3 = open(os.path.join(log_path, "ANN_record_lists.txt"), "w")
