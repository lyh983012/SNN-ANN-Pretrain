"""
train LIF from pretrained LIAF models
"""
...
from configs_LIF import training_configs, model_configs
...
ckpts_path = r'./checkpoints/LIF_from_LIAF_scratch_CRNN/'
tensorboard_path = r'./tensorboard/LIF_from_LIAF_scratch_CRNN/'
dic_save_path = r'./checkpoints/LIF_from_LIAF_scratch_CRNN/configs_LIF.txt'
log_path = r'./log/LIF_from_LIAF_scratch_CRNN/'
...
model_configs['encoder']['actFun'] = LIAF.LIFactFun.apply
model_configs['load_path'] = r'./checkpoints/LIAF_from_scratch2/liaf-epoch58-best.pth.tar'  # note: load checkpoints path
...
def main():
    # init model
    print('load checkpoints from: ' + model_configs['load_path'])
    ...
    cnn_encoder.load_state_dict(ckpts['cnn_encoder_state_dict'])
    rnn_decoder.load_state_dict(ckpts['rnn_decoder_state_dict'])
    # ===================================================================================================
    ...
    with open(os.path.join(log_path, "LIF_screen_log.txt"), "w") as f1:
        for epoch in range(1, training_configs['total_epoch'] + 1):
            ...
            if test_acc > best_test_acc:
                ...
                f2 = open(os.path.join(log_path, "LIF_best_test_acc.txt"), "w")
                ...
                if epoch >= training_configs['total_epoch'] / 2:
                    ...
                    torch.save(checkpoints, os.path.join(ckpts_path, 'lif-epoch{}-best.pth.tar'.format(epoch)))
            ...
            if epoch % training_configs['save_interval'] == 0 or epoch == training_configs['total_epoch']:
                ...
                torch.save(checkpoints, os.path.join(ckpts_path, 'lif-epoch{}.pth.tar'.format(epoch)))

    writer.close()

    f3 = open(os.path.join(log_path, "LIF_record_lists.txt"), "w")
